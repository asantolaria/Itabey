...
- `docs/fuentes/Marco general del proyecto.md` (CEO, general project framework)
- `docs/fuentes/Itabey-Pitch-Deck.pdf` (investor pitch deck)
- `docs/fuentes/Doc Pool.pdf` (project manifesto)

**Spanish canonical version:** `docs/design/prd.md`

**Changes since Rev 2:**
- **Phase structure reduced to 2** (previously 3): Phase 1 — MVP (full B2C + B2B core) and Phase 2 — Evolution (everything else). The merger reflects the General Framework § 6 clarification and the CEO–consultant conversation of 2026-05-27: the MVP launches commercially for B2C **and** initial B2B clients, but as a "solid, coherent and scalable core", not as a giant system.
- **Explicit MVP focus** (§ 1.4.0, CEO decision 2026-05-27): centred on Asha, tracking, insights, calendar, and UX. Explicitly **out of MVP** (architecturally prepared): community/forum (FR-501, FR-1004), shared panel (FR-305), 3D body map (FR-304).
- **Internal personas restructured into 5 profiles** with detailed dashboards and restrictions (Super Admin, Clinical Team, Moderation, Analytics, Senior Technical Supervision).
- **B2B corporate persona added** (P5 — Corporate Client) to reflect the new segment.
- **Primary market corrected to Spanish-speaking** (Spain + LATAM + Hispanic communities in the US), replacing "Spain + EU" from Rev 2.
- **New family of FRs (FR-1110 to 1114): integrations with external apps and deep links** — Asha recommends complementary apps (sleep, meditation, nutrition, fertility, neurodivergence, etc.), architecture prepared for strategic partnerships.
- **Male adaptation** included as Phase 2 vertical (future).
- **Owning entity and ecosystem** documented (§ 1.6) — Polymita Systems SL, Itabey, Asha.
- **Increased functional detail** at the request of the development team: every FR carries acceptance criteria, priority, delivery phase, and explicit dependencies.
...
| NG3 | HL7/FHIR compliance in MVP | Out of MVP. **It is required** that the architecture allow later mapping without structural redesign — see § 4.10, NFR-M, E15 in § 12.2 |
...
- **Standards mapping:** HL7/FHIR not required in MVP, but the internal structure must allow mapping to these standards in later phases (see § 4.10 NFR-M09, E15 in § 12.2).
...
- **Acceptance criteria:**
  ...
  - Minimum cohort size for aggregate reports: ≥ 10 active users (proposal, validable by CEO in Q10).
...
| NFR-D03 | Subprocessor documentation with international transfer justification if any. **Attention:** expansion to LATAM and US (NFR-I01) requires specific analysis of international transfers — CEO open question Q2 |
...
| R4 | LLM inference costs grow above expectations | M | Medium | Per-scenario estimation, caching, layered models (large model only when RAG justifies), per-tier quotas (FR-1306, NFR-SC07) |
...
> These questions need a CEO answer before closing the vendor evaluation phase. Some affect architecture and metrics. Recommendation: group them in a single conversation.

| ID | Question | Why it matters | Status |
|----|----------|----------------|--------|
| Q1 | What traction hypothesis has been presented to investors in the seed (€1.5M)? | Without it, § 7 KPIs are generic benchmarks, not validated commitments | Open |
| Q2 | Is initial deployment focused on Spain, with gradual expansion to LATAM and US Hispanics on what horizon? | Affects GDPR compliance, international transfers, infrastructure costs | Open |
| Q3 | Is the multidisciplinary clinical team (PI2) already identified/contracted, or is helping to recruit it part of the vendor's scope? | Substantially changes budget and schedule | Open |
| Q4 | Does the project podcast already exist with produced content, or is it created alongside the product? | If absent, FR-503 leaves the MVP | Open |
| Q5 | Sports vertical: 12–18 month or 24+ month post-MVP horizon? | Determines architectural surface to reserve | Open |
| Q6 | The defensible asset is the **combination of common core (UX + knowledge base + RAG engine) and modular tiering capability**. Confirm this reading? | Validates the architectural emphasis of § 1.5, NFR-SC06–07, and § 12.4 evaluation criteria | Open |
| Q7 | Is there a cost ceiling for the MVP, or is the proposal open to vendor? | Defines a realistic MVP scope | Open |
| Q8 | LLM model policy: private, open-source self-hosted, mixed? | Critical implications on privacy and cost | Open |
| Q9 | When will the **concrete tier contents** be defined? Suggestion: after 3 months of real usage with a validation cohort. | Allows product/commercial to plan the decision without blocking development | Open |
| Q10 | What minimum B2B cohort is acceptable to guarantee aggregate privacy (FR-1304)? MVP proposal: ≥ 10 active users for aggregate reports. | Below a certain size, re-identification by inference becomes feasible | Open |
| Q11 | Is any specific B2B client already identified for the MVP pilot (FR-1006), or is it sought during development? | Changes the level of detail required in MVP corporate dashboards and urgency of enterprise SSO | Open |
| Q12 | For the initial recommendable external app catalogue (FR-1111): is any app or partner already identified, or built from scratch? | Determines whether FR-1111 can activate early in Phase 2 or requires prospecting phase | Open |
| Q13 | Is there reserved budget for explanatory video production (FR-702), or is it delegated to the development vendor? | Changes cost and timing of deliverable E13 | Open |
| Q14 | Male adaptation (Phase 2): distinct brand or maintained under the Itabey umbrella? | Determines visual architecture decisions, multi-tenant, and commercial choices | Open |
